...
void deleteFile(String filename) {
  ...
  if (transmitting && currentLoRaFile == filename) { sendResponse("ERROR:FILE_IN_USE"); return; }
...
void setLoRaConfig(String jsonStr) {
  ...
  if (doc.containsKey("bw")) {
    ...
    if (bw == 125.0 || bw == 250.0 || bw == 500.0) {
      ...
      Serial.printf("⚠️  BW inválido (%.1f), ignorado\n", bw);
    }
  }
  ...
  if (doc.containsKey("sf")) {
    ...
    if (sf >= 5 && sf <= 12) {
      ...
      Serial.printf("⚠️  SF inválido (%d), ignorado\n", sf);
    }
  }
  ...
  if (doc.containsKey("cr")) {
    ...
    if (cr >= 5 && cr <= 8) {
      ...
      Serial.printf("⚠️  CR inválido (%d), ignorado\n", cr);
    }
  }
  ...
  if (doc.containsKey("repeat")) {
    ...
    if (rep >= 1 && rep <= 20) {
      ...
      Serial.printf("⚠️  REPEAT inválido (%d), ignorado\n", rep);
    }
  }
